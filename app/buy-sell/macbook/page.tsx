...
const HeroSection = memo(function HeroSection() {
  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      ...
      <h1 className="text-2xl font-bold text-white">MacBook</h1>
...
const MobileContent = memo(function MobileContent() {
  ...
  const macbooks = products.filter(p => p.category === "macbook");
  
  const sortedMacbooks = [...macbooks].sort((a, b) => {
...
const MacBook3DViewer = dynamic(() => import("@/components/macbook-3d"), {
  ssr: false,
  loading: () => (
...
export default function MacBookBuySellPage() {
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
    ...
    <main className="min-h-screen py-24 px-6 bg-main">
          ...
          <div>
            <div className="flex items-center justify-center lg:justify-start gap-4 mb-4">
              ...
              <h1 className="text-5xl md:text-6xl font-bold text-center lg:text-left">
                <span className="bg-black/60 px-4 py-2 rounded-xl text-white">MacBook</span>
              </h1>
            </div>
            <p className="text-gray-400 text-center lg:text-left mt-4 text-lg">Buy & Sell premium MacBooks - Select storage</p>
          </div>
          <div className="h-40 lg:h-60 rounded-xl overflow-hidden shadow-xl">
            <video autoPlay loop muted playsInline preload="none" className="w-full h-full object-cover" poster="/images/macbook.jpg">
      ...
      {selectedProduct ? (
        <div className="container mx-auto px-6">
          <button onClick={() => { setSelectedProduct(null); setSelectedStorage(""); setShow3D(false); }} className="mb-6 text-amber-600 hover:text-amber-500 flex items-center gap-2">← Back to all MacBooks</button>
          
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <GlassCard className="p-4">
              {show3D ? (<div className="h-64 md:h-80 w-full"><MacBook3DViewer /></div>) : (
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className="text-gray-400 mb-4 text-sm">Premium quality MacBook available for sale. Each device is thoroughly inspected and certified.</p>
