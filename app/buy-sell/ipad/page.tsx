...
const HeroSection = memo(function HeroSection() {
  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      ...
      <h1 className="text-2xl font-bold text-white">iPad</h1>
...
const MobileProductDetail = memo(function MobileProductDetail({ product, onBack, onWhatsApp }: { product: Product; onBack: () => void; onWhatsApp: (action: "buy" | "sell") => void }) {
  ...
  return (
    <div className="px-4 py-4">
      <button onClick={onBack} className="mb-4 text-amber-500 flex items-center gap-2 text-sm">
        ← Back
      </button>
      ...
      {product.storage && product.storage.length > 0 && (
        <div className="mb-4">
          <p className="text-gray-400 text-sm mb-2 text-center">Select Storage</p>
          <div className="flex flex-wrap justify-center gap-2">
            {product.storage.map((storage) => (
              <button key={storage} onClick={() => setSelectedStorage(storage)}
                className={`px-4 py-2 rounded-lg text-sm ${selectedStorage === storage ? "bg-amber-600 text-white" : "bg-white/10 text-gray-300"}`}>
                {storage}
              </button>
            ))}
          </div>
      ...
      <div className="flex gap-3 mt-6">
        <button onClick={() => onWhatsApp("buy")} className="flex-1 py-3 bg-green-600 text-white rounded-lg font-semibold text-sm active:scale-95 transition-transform">
          Buy Now
        </button>
        <button onClick={() => onWhatsApp("sell")} className="flex-1 py-3 bg-amber-600 text-white rounded-lg font-semibold text-sm active:scale-95 transition-transform">
          Sell Yours
        </button>
      </div>
    </div>
    ...
    if (selectedProduct) {
      return (
        <div className="bg-black text-white min-h-screen">
          <MobileHeader />
          <HeroSection />
          <MobileProductDetail product={selectedProduct} onBack={() => { setSelectedProduct(null); setSelectedStorage(""); }} onWhatsApp={handleWhatsApp} />
        </div>
      );
    }
    return (
    ...
    <main className="min-h-screen py-24 px-6 bg-main">
      <div className="container mx-auto px-6 mb-16">
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <p className="text-gray-400 text-center lg:text-left mt-4 text-lg">Buy & Sell premium iPads - Select storage</p>
          </div>
          <div className="h-40 lg:h-60 rounded-xl overflow-hidden shadow-xl">
            <video autoPlay loop muted playsInline preload="none" className="w-full h-full object-cover" poster="/images/ipad%20Air%2011%20in(M4)/ipad%20Air%2011%20in(M4)-blue.jpg">
      ...
      {selectedProduct ? (
        <div className="container mx-auto px-6">
          <button onClick={() => { setSelectedProduct(null); setSelectedStorage(""); setShow3D(false); }} className="mb-6 text-amber-600 hover:text-amber-500 flex items-center gap-2">
            ← Back to all iPads
          </button>
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            <div className="flex flex-col justify-center">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProduct.storage.map((storage) => (
                      <button key={storage} onClick={() => setSelectedStorage(storage)} className={`px-4 py-2 rounded-lg text-sm transition-all ${selectedStorage === storage ? "bg-amber-600 text-white" : "bg-white/10 text-gray-300 hover:bg-white/20"}`}>
                        {storage}
                      </button>
                    ))}
                  </div>
              ...
              <p className="text-gray-400 mb-4 text-sm">Premium quality iPad available for sale. Each device is thoroughly inspected and certified.</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {sortedIpads.map((product) => (
              <GlassCard key={product.id} onClick={() => handleProductSelect(product)} className="p-4 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-amber-600/20">
                ...
                {product.storage && product.storage.length > 0 && (
                  <p className="text-center text-gray-500 text-xs mt-1">{product.storage[0]} - {product.storage[product.storage.length - 1]}</p>
                )}
              </GlassCard>
            ))}
          </div>
