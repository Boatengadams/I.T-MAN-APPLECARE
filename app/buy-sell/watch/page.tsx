...
const HeroSection = memo(function HeroSection() {
  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      ...
      <h1 className="text-2xl font-bold text-white dark:text-black">Apple Watch</h1>
...
const MobileProductDetail = memo(function MobileProductDetail({ product, onBack, onWhatsApp }: { product: Product; onBack: () => void; onWhatsApp: (action: "buy" | "sell") => void }) {
  ...
  return (
    <div className="px-4 py-4">
      ...
      {product.storage && product.storage.length > 0 && (
        <div className="mb-4">
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-2 text-center">Select Size</p>
          <div className="flex flex-wrap justify-center gap-2">
            {product.storage.map((size) => (
              <button key={size} onClick={() => setSelectedSize(size)}
                className={`px-4 py-2 rounded-lg text-sm ${selectedSize === size ? "bg-amber-600 text-white" : "bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-300"}`}>
                {size}
              </button>
            ))}
          </div>
  ...
  const handleProductSelect = (product: Product) => {
   ...
   const handleWhatsApp = (action: "buy" | "sell") => {
     ...
     const productData = {
       ...
       spec: selectedSize,
       color: "Not Selected", // Watch page doesn't have color selection in mobile view
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <GlassCard className="p-4">
              {show3D ? (<div className="h-64 md:h-80 w-full"><Watch3DViewer productId={selectedProduct.id} /></div>) : (
