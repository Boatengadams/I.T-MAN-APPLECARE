...
const HeroSection = memo(function HeroSection() {
  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      ...
      <h1 className="text-2xl font-bold text-white">iMac</h1>
...
const MobileProductDetail = memo(function MobileProductDetail({ product, onBack, onWhatsApp }: { product: Product; onBack: () => void; onWhatsApp: (action: "buy" | "sell") => void }) {
  ...
  return (
    <div className="px-4 py-4">
      ...
      {product.storage && product.storage.length > 0 && (
        <div className="mb-4">
          <p className="text-gray-400 text-sm mb-2 text-center">Select Configuration</p>
          <div className="flex flex-wrap justify-center gap-2">
            {product.storage.map((config) => (
              <button key={config} onClick={() => setSelectedConfig(config)}
                className={`px-4 py-2 rounded-lg text-sm ${selectedConfig === config ? "bg-amber-600 text-white" : "bg-white/10 text-gray-300"}`}>
                {config}
              </button>
            ))}
          </div>
