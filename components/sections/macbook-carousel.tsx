...
import { macbooks } from "@/lib/products";

function generateCarouselItems(items: typeof macbooks, count: number): typeof macbooks {
  const result: typeof macbooks = [];
...
export function MacBookCarousel() {
  ...
  const [carouselItems, setCarouselItems] = useState<typeof macbooks>([]);

  useEffect(() => {
    setCarouselItems(generateCarouselItems(macbooks, 6));
  }, []);
