...
function generateCarouselItems(items: typeof watches, count: number): typeof watches {
  const result: typeof watches = [];
...
export function WatchCarousel() {
  ...
  const [carouselItems, setCarouselItems] = useState<typeof watches>([]);

  useEffect(() => {
    setCarouselItems(generateCarouselItems(watches, 6));
  }, []);
